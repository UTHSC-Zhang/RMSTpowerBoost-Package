...
knitr::opts_chunk$set(
  collapse = TRUE,
  cache = FALSE,
  cache.path = "cache/RMSTpowerBoost-DataGen/",
  cache.lazy = FALSE,
  autodep = TRUE,
  comment = NA,
...
For common cases — AFT or PH baseline, random censoring, standard covariate distributions — `rmst.sim()` provides a single-call interface. Use `covar_continuous()`, `covar_binary()`, and `covar_categorical()` to specify covariates without building nested lists by hand.

```{r quick_start, eval=TRUE}
df <- rmst.sim(
  n              = 200,
  model          = "aft_lognormal",
...
For designs requiring explicit covariate-dependent censoring, stratified assignment, or frailty, use the full recipe system described below.
