...
The design and analysis of clinical trials with time-to-event endpoints often rely on the hazard-based model (e.g., the Cox model) and use the hazard ratio (HR) as the primary estimand of the treatment effect. However, the HR can be challenging to interpret from both clinical and causal perspectives. Moreover, the key assumption underlying most hazards models - the proportional hazards assumption - is often tenuous or violated in real-world settings.
