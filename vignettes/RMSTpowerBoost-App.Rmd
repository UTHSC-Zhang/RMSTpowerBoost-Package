...
knitr::opts_chunk$set(
  ...
  warning = FALSE,
  cache = FALSE,
  cache.path = "cache/RMSTpowerBoost-App/"
)
```

# Introduction

This vignette describes the current `RMSTpowerBoost` Shiny application. The app exposes the package's RMST-based design methods through a workflow-oriented interface for either uploaded pilot data or app-generated simulated data.

# Workflow Overview

The application is organized as a staged workflow in the sidebar and a results workspace in the main panel.
...
- Each step has explicit `Confirm` and `Reset` buttons; there is also a workflow-level `Reset` button at the top of the app.
...
The main panel is organized into the tabs shown in the shipped UI.

## Pipeline

This tab summarizes the current workflow state and is the landing page when the app opens or resets.
...
The `About` tab summarizes the app purpose and package context.
