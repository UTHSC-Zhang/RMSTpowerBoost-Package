...
This vignette describes the RMSTpowerBoost Shiny application. The app provides a graphical interface to the package's RMST-based power and sample size procedures.

The guide summarizes the main inputs, parameters, and outputs used in the application.
...
6.  **Download Report:** Generate a PDF report of your analysis.
...
This is the starting point of your analysis.
  ...
  * **Select RMST Model:** Choose the statistical model that best fits your trial's design and assumptions. The available models are:
      ...
      * **`Dependent Censoring Model`**: For studies where censoring may depend on measured covariates under a single censoring mechanism.
...
This panel contains the core settings for the calculation itself.
  ...
  * **Calculation Method:** (Only appears for `Linear IPCW` and `Multiplicative Stratified` models)
      * **`Analytical`**: A fast method based on a direct mathematical formula.
      * **`Bootstrap`**: A simulation-based method that is more computationally intensive.
  * **Truncation Time (`L`):** The truncation time for the RMST calculation. This should be a clinically meaningful time point within the follow-up period of your study (e.g., `365` for 1-year RMST).
  ...
  * **Significance Level (`alpha`):** The Type I error rate, typically set to 0.05.
