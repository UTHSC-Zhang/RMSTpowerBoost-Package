...
system.time(
  results_power_linear <- design_rmst_linear_ipcw(
...
# Set parallel.cores > 1 to enable parallel processing for more intensive searches.
# system.time(
#   results_n_linear <- design_rmst_linear_ipcw(
#     pilot_data = lung_pilot,
#     time_var = "time",
#     status_var = "status",
#     arm_var = "sex",
#     target_powers = c(0.20, 0.30),
#     linear_terms = NULL,
#     tau = lung_tau,
#     n_sim = 200,
#     parallel.cores = 10
...
```{r pbc_data_prep_comp_risk, include=TRUE}
pbc_pilot_comp_risk <- pbc %>%
  dplyr::mutate(
    # Create the 'transplant' indicator for the competing event (status = 1)
    transplant = ifelse(status == 1, 1, 0),
    # Recode 'status' to represent only the primary event (death, status = 2)
    status = ifelse(status == 2, 1, 0),
    # Recode 'trt' to the required 0/1 format
    trt = ifelse(trt == 1, 1, 0)
  ) %>%
  tidyr::drop_na(time, status, trt, transplant, age, bili)
```
...
system.time(
  results_power_cr <- design_rmst_dependent_censoring(
    pilot_data = pbc_pilot_comp_risk,
    time_var = "time",
    status_var = "status",
    arm_var = "trt",
    dep_cens_status_var = "transplant",
...
system.time(
  results_sample_cr <- design_rmst_dependent_censoring(
    pilot_data = pbc_pilot_comp_risk,
    time_var = "time",
    status_var = "status",
    arm_var = "trt",
