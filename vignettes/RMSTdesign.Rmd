...
# --- 1. Data Preparation: Create a robust pilot dataset ---
# This dataset has a larger size and higher event rates to ensure stability
# for the dependent censoring model.
set.seed(42)
n_pilot <- 300 # Larger pilot dataset
pilot_data_cr <- data.frame(
  time = c(stats::rexp(n_pilot/2, 0.1), stats::rexp(n_pilot/2, 0.05)), #<-- Stronger effect
  arm = rep(0:1, each = n_pilot/2),
  age = stats::rnorm(n_pilot, 65, 5)
)
event_probs <- stats::runif(n_pilot)
# Higher event rates
pilot_data_cr$primary_event <- ifelse(event_probs < 0.40, 1, 0) # 40% primary events
pilot_data_cr$competing_event <- ifelse(event_probs >= 0.40 & event_probs < 0.75, 1, 0) # 35% competing
pilot_data_cr$primary_event[pilot_data_cr$competing_event == 1] <- 0
