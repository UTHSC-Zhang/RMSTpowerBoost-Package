...
The design and analysis of clinical trials with time-to-event endpoints often rely on hazard-based methods such as the proportional hazards (PH) model and use the hazard ratio (HR) as the primary estimand of treatment effect. However, the HR can be difficult to interpret clinically, especially when treatment effects vary over time. Moreover, the key assumption underlying most hazard-based methods - proportional hazards - is often tenuous or violated in practice.

As an alternative, the **Restricted Mean Survival Time (RMST)** provides a direct summary of average event-free time up to a pre-specified follow-up point, $L$ [@royston2013; @uno2014]. This yields an interpretable treatment contrast, for example, an average gain of several months of survival over a clinically relevant horizon.

Modern statistical methods also model the RMST directly as a function of baseline covariates rather than estimating it only from a survival curve or hazard model. This direct approach, based on work using Inverse Probability of Censoring Weighting (IPCW) [@tian2014], has been extended to accommodate stratification in multi-center studies [@wang2019; @zhang2024] and covariate-dependent censoring [@wang2018].
...
The `RMSTpowerBoost` package addresses this design problem by providing tools for **power and sample size calculations** based on direct RMST methodologies from the recent literature. The package implements several key approaches:
...
This vignette summarizes the theory and application of each of these function groups.
...
The analytical functions are fast because they use a direct mathematical formula to calculate power. This makes them useful for exploring different scenarios. The process is:
...
The bootstrap functions provide a simulation-based alternative that relies less on closed-form analytic approximations. This is a trade-off, as they are much more computationally intensive. The process is:
...
## Available Functions: A Quick Guide
...
| **Dependent Censoring**       | Accounts for covariate-dependent censoring under a single censoring mechanism. | Recommended when censoring depends on measured covariates and the competing-risks setting is not being modeled explicitly. |
...
The `.boot` suffix in function names indicates a bootstrap, or simulation-based, approach. This method repeatedly resamples from the pilot data, fits the model on each sample, and calculates power as the proportion of simulations where the treatment effect is significant. While computationally intensive, it relies less on closed-form large-sample approximations.
...
While the formal estimation of $\beta$ requires a complex iterative solver, this package uses a practical and computationally efficient approximation. It fits a weighted log-linear model (`lm(log(Y_rmst) ~ ...)`) to the data, which provides an approximate estimate of the effect size (the log-RMST ratio) and its variance.
...
The bootstrap approach provides a simulation-based analysis for the multiplicative model.
...
1.  **Jackknife Pseudo-Observations**: The time-to-event outcome is first converted into **jackknife pseudo-observations** for the RMST. This technique, explored in the statistical literature for RMST estimation [@andersen2004; @parner2010], creates a continuous, uncensored variable that represents each subject's contribution to the RMST. This makes the outcome suitable for use in a standard regression framework.
...
\]
where \(\widehat{\mathrm{Var}}(\hat\beta_A)\) is the asymptotic variance estimated from the pilot data and \(N\) is the total sample size.  
...
This function estimates analytic power for the RMST regression under a single covariate-dependent censoring mechanism.
...
kbl(ss_dc_mgus$results_summary,
    caption = "Estimated RMST Difference from Pilot Data") %>%
  kable_styling(bootstrap_options = "striped",
                full_width = FALSE, position = "center")
...
For users who prefer a graphical interface, `RMSTpowerBoost` provides an interactive Shiny web application that exposes the models and methods described in this vignette.
...
-   **Interactive Data Generation**: Upload your pilot dataset in `.csv` format or generate data for analysis.
...
-   **Full Model Selection**: Choose the RMST model, calculation method (analytical or bootstrap), and the associated parameters through the interface.
-   **Rich Visualization**: Execute the analysis and review the resulting data summary, Kaplan-Meier plot, analysis tables, and power curve within the application.
-   **Downloadable Reports**: Export the completed analysis as either PDF or HTML.

# Conclusion

The `RMSTpowerBoost` package provides functions for RMST-based power and sample size calculations under linear, stratified, semiparametric, and covariate-dependent censoring models.

In practice, these procedures depend on representative pilot data because effect sizes and variance components are estimated from that dataset. Bootstrap-based methods can also require substantial computation.

Future development could extend simulation-based procedures for covariate-dependent censoring and add more model diagnostic tools for checking the adequacy of pilot-data assumptions.
