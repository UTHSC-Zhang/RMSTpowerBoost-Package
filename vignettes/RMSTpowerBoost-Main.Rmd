...
-   **Interactive Data Generation**: Upload your reference dataset in `.csv` format or generate data for analysis.
...
The `RMSTpowerBoost` package provides a unified formula-based interface — `rmst.power()` and `rmst.ss()` — for RMST-based power and sample size calculations under linear, stratified, semiparametric, and covariate-dependent censoring models. Both functions accept the same `Surv(time, status) ~ covariates` formula syntax used throughout the `survival` ecosystem, with model selection controlled by a small set of arguments (`strata`, `strata_type`, `dep_cens`, `type`).
...
Future development could extend simulation-based procedures for covariate-dependent censoring and add more model diagnostic tools for checking the adequacy of reference-data assumptions.
