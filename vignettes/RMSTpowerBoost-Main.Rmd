...
For simulation-driven workflows, `rmst.sim()` provides matching pilot data with the same unified design. The `seed` argument is stored in the recipe metadata for provenance, while reproducible simulation still comes from calling `set.seed()` before `rmst.sim()`.
