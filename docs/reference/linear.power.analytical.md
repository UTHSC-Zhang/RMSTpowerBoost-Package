...
linear model of the form \$\$E\[Y_i \mid A_i, \mathbf{Z}\_i\] = \alpha +
\tau A_i + \mathbf{Z}\_i^\top \boldsymbol{\gamma}\$\$ where \\Y_i =
\min(T_i, L)\\ is the event time truncated at \\L\\, \\A_i\\ is the
treatment indicator, and \\\tau\\ is the treatment effect of interest.
...
estimator, \\\hat{\tau}\\, is derived from a robust sandwich variance
