...
\\\mu\_{ij} = \mu\_{0j} + \beta'Z_i\\, where `i` indexes subjects and
`j` indexes strata.
...
regression coefficient \\\hat{\beta}\\ is estimated by centering the
...
\\\hat{\beta}\\. This implementation uses the robust variance estimator
\\A_n^{-1} B_n (A_n^{-1})'\\, where \\A_n\\ and \\B_n\\ are empirical
