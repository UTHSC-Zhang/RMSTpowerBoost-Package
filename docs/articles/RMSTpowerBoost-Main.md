...
quantity $`\beta_{\text{effect}}`$ therefore defines the **effect
size**. All analytical power and sample size functions in
`RMSTpowerBoost` use this parameterization as their foundation.
...
strata $`j`$. Crucially, it allows each stratum to have its own unique
baseline RMST, denoted by $`\mu_{0j}`$.
...
numerous $`\mu_{0j}`$ parameters, making it computationally efficient
even with a large number of strata.
...
--- Searching for Sample Size (Method: Multiplicative Stratified RMST Model) ---
  N = 100/stratum, Calculating Power... Power = 0.35
  N = 150/stratum, Calculating Power... Power = 0.5
...
The variance of $`\hat\beta_A`$ is estimated with a sandwich-type
estimator that treats the censoring model as known.
...
estimated from the pilot data and $`N`$ is the total sample size.  
